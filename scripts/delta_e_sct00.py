#!/usr/bin/env python3
"""
ΔE_SCT00 — Enhanced Color Difference: CIEDE2000 + Fisher Correction
=====================================================================

The SCT00 formula combines CIEDE2000's empirical cortical model with
the Fisher-Bernoulli luminance geodesic derived from Persistence Theory.

Formula:
    ΔE²_SCT00 = w₁·ΔE₀₀ + w₂·d_lum + w₃·ΔE₀₀² + w₄·ΔE₀₀·d_lum + w₅·d_lum²
...
Usage:
    from delta_e_sct00 import delta_e_sct00, fit_sct00, SCT00Model

    # Fit on COMBVD (or your own dataset)
    model = fit_sct00()

    # Predict
    de = delta_e_sct00(Lab1, Lab2, xyz1, xyz2, model)
...
@dataclass
class SCT00Model:
    """Fitted SCT00 model coefficients."""
    ...
    def __repr__(self):
        return (f"SCT00Model(r={self.r_cv:.4f}, "
...
def fit_sct00(combvd_path=None):
    """
    Fit the SCT00 model on COMBVD.

    Returns:
        SCT00Model with fitted weights.
    """
    import pandas as pd
...
def delta_e_sct00(Lab1, Lab2, Y1, Y2, model=None):
    """
    Compute ΔE_SCT00 color difference.

    Parameters:
        Lab1, Lab2: CIELAB (L*, a*, b*)
        Y1, Y2: CIE Y tristimulus (luminance)
        model: fitted SCT00Model (if None, uses default weights)

    Returns:
        ΔE_SCT00 (float or array)
    """
...
def validate(verbose=True):
    ...
    r_sct00 = stats.pearsonr(y_pred, dv)[0]
    ...
    if verbose:
        print("=" * 65)
        print("ΔE_SCT00 — Validation Report")
        print("=" * 65)
        ...
        print(f"  │ ΔE_SCT00 (this work)    │ {r_sct00:.4f} │      5 │")
        print(f"  └─────────────────────────┴────────┴────────┘")

        print(f"\n  Improvement over CIEDE2000: Δr = +{r_sct00 - r_de00:.4f}")
        ...
        for name, mask in regions.items():
            ...
            print(f"    {name:20s} (n={n:4d}): "
                  f"CIEDE2000 r={r1:.3f}  SCT00 r={r2:.3f}  Δ={r2-r1:+.3f}")

        print(f"\n  Formula:")
        print(f"    ΔE_SCT00 = {b_raw:.4f}")
    ...
    return {
        'r_sct00': r_sct00, 'r_ciede2000': r_de00, 'r_cielab': r_lab,
