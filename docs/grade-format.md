...
A `.scs` file stores a color grade as a displacement field on the SCS
simplex. Unlike `.cube` files (which are RGB->RGB tables tied to a specific
color space), a `.scs` grade is **gamut-independent**: it can be captured in
...
Use ChromaPlex (tab 3: Convert .cube) with the "Export .scs" checkbox,
or programmatically:
